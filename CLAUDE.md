...
The dissertation work is complete (results in `dissertation_results/`). The codebase is in post-dissertation maintenance mode. Two cleanup commits in May 2026 (`7b9e987`, `c0748ab`) freed ~17 GB by pruning training artifacts, log directories, orphan demo variants (`demo_dqn_simple.py`, `demo_dqn_learning_fixed.py`, `demo_ab_testing_rewards.py`, `simple_pattern_demo.py`, `hello_mcp.py`, `run_demo5_only.py`), and unused config experiments. Rollback tags: `pre-cleanup-20260510`, `pre-cleanup-followup-20260510`.
...
# 4. Verify imports (no verify_setup.py exists; use a quick smoke import)
python -c "from src.agents import OrchestratorAgent; from src.core.mcp_integration import MCPIntegration; print('imports OK')"
```
...
- **Quick Launch**: `cd src/web && uvicorn demo_app:app --reload --port 8000` then navigate to http://localhost:8000
